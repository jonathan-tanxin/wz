                    ...
                    <div class="ibox-content">
                    	<form role="form" class="form-inline" id="query-form" method="post" action="${ctx }/admin/order/sendList" >
                            ...
                            <div class="form-group">
                                ...
                                <select placeholder="用户状态" id="query-status" name="status" value="${query.status }" class="form-control">
						...
						<div>
							<p>
		                        ...
		                        <!-- -->
		                        <button type="button" class="btn btn-outline btn-danger" onclick="setSend();">设置配送</button>
									<button type="button" class="btn btn-outline btn-danger" onclick="exportExcel();">导出</button>
		                        </sec:authorize>
		                    </p>
						</div>
						<table id="table" data-toggle="table" data-height="400" data-click-to-select="true" data-mobile-responsive="true">
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-field="address.name">收货人姓名</th>
                                            <th data-field="address.mobile">收货人手机号</th>
                                            <th data-field="address.address">收货人地址</th>
                                            <th data-field="address.distributionCenter.name">配送中心</th>
							...
							<tbody>
								...
								<tr data-index="${idx.index }">
									...
									<td style="">${n.address.name }</td>
									<td style="">${n.address.mobile }</td>
									<td style="">${n.address.address }</td>
									<td style="">${n.address.distributionCenter.name }</td>
...
function setSend(){
	...
	layer.open({
	    ...
	    skin: 'layui-layer-rim', //加上边框
	    area: [layer_default_width, layer_default_height], //宽高
	    content: "${ctx}/admin/order/setSend?id=" + id,
	    btn: ['确定', '取消'],
	    yes: function(index, layero){ //或者使用btn1
	    	var detailForm = layer.getChildFrame('form', index);
	    	console.log(detailForm);
...
function exportExcel(){
    ...
    var nickName = $("#query-customer").val();
    var orderNo = $("#query-orderNo").val();
    var status = $("#query-status").val();
    var distributeStatus = $("#query-distributeStatus").val();
    var url = "${ctx}/admin/order/exportExcel";
    url += "?customer.nickName="+nickName;
    url += "&orderNo=" + orderNo;
    url += "&status=" + status;
    url += "&distributeStatus=" + distributeStatus;
    url += "&ids="+ids.join(",");
    window.open(url);
}
...
function edit() {
	...
	layer.open({
	    ...
	    skin: 'layui-layer-rim', //加上边框
	    area: [layer_default_width, layer_default_height], //宽高
	    content: "${ctx}/admin/orderItem/list?order.id=" + id,
	    btn: ['确定', '取消'],
	    yes: function(index, layero){ //或者使用btn1
	    	var detailForm = layer.getChildFrame('form', index);
	    	console.log(detailForm);

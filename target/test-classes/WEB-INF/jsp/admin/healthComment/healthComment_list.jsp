            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-content">
                    	<form role="form" class="form-inline" id="query-form" method="post" action="${ctx }/admin/healthComment/list" >
                    		<!-- 查询开始 -->
						...
						<div>
							<p>
								<sec:authorize ifAnyGranted="healthComment">
						...
						<table id="table" data-toggle="table" data-height="400" data-click-to-select="true" data-mobile-responsive="true">
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-field="expertCustomer">专家</th>
                                            <th data-field="content">点评内容</th>
                                            <th data-field="createTime">点评时间</th>
                                        </tr>
                                    </thead>
							<tbody>
								...
								<tr data-index="${idx.index }">
									...
									<td style="">${n.expertCustomer.name}</td>
...
function add() {
		layer.open({
		    ...
		    content: "${ctx}/admin/healthComment/detail",
		    btn: ['确定', '取消'],
...
function edit() {
	...
	layer.open({
	    ...
	    skin: 'layui-layer-rim', //加上边框
	    area: [layer_default_width, layer_default_height], //宽高
	    content: "${ctx}/admin/healthComment/detail?id=" + id,
	    btn: ['确定', '取消'],
	    yes: function(index, layero){ //或者使用btn1
	    	var detailForm = layer.getChildFrame('form', index);
	    	console.log(detailForm);
...
function del() {
	...
	layer.confirm('确认要删除吗?', {icon: 3, title:'提示'}, function(index){
		...
		$.ajax({
			url: "${ctx}/admin/healthComment/delete",
			data: {
				ids: ids.join(',')
			},
			success: function(data) {

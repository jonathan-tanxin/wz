            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-content">
                    	<form role="form" class="form-inline" id="query-form" method="post" action="${ctx }/admin/authority/list" >
                    		<!-- 查询开始 -->
						...
						<table id="table" data-toggle="table" data-height="400" data-click-to-select="true" data-mobile-responsive="true">
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-field="parentAuthority">上级权限</th>
                                            <th data-field="name">名称</th>
                                            <th data-field="code">编码</th>
                                        </tr>
                                    </thead>
							<tbody>
								<c:forEach items="${authorities }" var="n" varStatus="idx">
								<tr data-index="${idx.index }">
									<td class="bs-checkbox">
										<input data-index="0" name="btSelectItem" type="checkbox">
									</td>
									<td style="">${n.id }</td>
									<td style="">${n.parentAuthority.name}</td>
									<td style="">${n.name }</td>
									<td style="">${n.code }</td>
								</tr>
								</c:forEach>
...
function del() {
	...
	layer.confirm('确认要删除吗?', {icon: 3, title:'提示'}, function(index){
		...
		$.ajax({
			url: "${ctx}/admin/authority/delete",
			data: {
				ids: ids.join(',')
			},
			success: function(data) {

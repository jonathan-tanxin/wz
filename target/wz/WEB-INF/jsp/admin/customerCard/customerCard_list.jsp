            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-content">
                    	<form role="form" class="form-inline" id="query-form" method="post" action="${ctx }/admin/customerCard/list" >
                    		<!-- 查询开始 -->
                            ...
                            <div class="form-group">
                                <label for="query-card" class="sr-only">会员卡</label>
                                	<input type="text" placeholder="会员卡" id="query-card" name="card" value="${query.card }" class="form-control">
                            </div>
                            <div class="form-group">
						...
						<div>
							<p>
								<sec:authorize ifAnyGranted="customerCard">
						...
						<table id="table" data-toggle="table" data-height="400" data-click-to-select="true" data-mobile-responsive="true">
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-field="card">会员卡</th>
                                            <th data-field="money">购买金额</th>
							...
							<tbody>
								...
								<tr data-index="${idx.index }">
									...
									<td style="">${n.money }</td>
									<td style="">
										<fmt:formatDate value="${n.startTime }" pattern="yyyy-MM-dd HH:mm:ss"/>
									</td>
									<td style="">
										<fmt:formatDate value="${n.endTime }" pattern="yyyy-MM-dd HH:mm:ss"/>
									</td>
									<td style="">
...
function add() {
		layer.open({
		    ...
		    content: "${ctx}/admin/customerCard/detail",
		    btn: ['确定', '取消'],
...
function edit() {
	...
	layer.open({
	    ...
	    skin: 'layui-layer-rim', //加上边框
	    area: [layer_default_width, layer_default_height], //宽高
	    content: "${ctx}/admin/customerCard/detail?id=" + id,
	    btn: ['确定', '取消'],
	    yes: function(index, layero){ //或者使用btn1
	    	var detailForm = layer.getChildFrame('form', index);
	    	console.log(detailForm);
...
function del() {
	...
	layer.confirm('确认要删除吗?', {icon: 3, title:'提示'}, function(index){
		...
		$.ajax({
			url: "${ctx}/admin/customerCard/delete",
			data: {
				ids: ids.join(',')
			},
			success: function(data) {

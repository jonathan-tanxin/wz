            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-content">
                    	<form role="form" class="form-inline" id="query-form" method="post" action="${ctx }/admin/orderItem/list" >
                    		<!-- 查询开始 
                            <div class="form-group">
                                <label for="query-order" class="sr-only">订单</label>
                                	<input type="text" placeholder="订单" id="query-order" name="order" value="${query.order }" class="form-control">
                            </div>
                            <div class="form-group">
                                <label for="query-type" class="sr-only">购买类型</label>
                                	<input type="text" placeholder="购买类型" id="query-type" name="type" value="${query.type }" class="form-control">
                            </div>
                            <div class="form-group">
						...
						<div>
							<p>
								<sec:authorize ifAnyGranted="orderItem">
								<!-- 
						...
						<table id="table" data-toggle="table" data-height="400" data-click-to-select="true" data-mobile-responsive="true">
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-field="order">订单</th>
                                            <th data-field="dish">菜品</th>
                                            <th data-field="amount">数量</th>
                                            <th data-field="type">购买类型</th>
                                        </tr>
                                    </thead>
							<tbody>
								...
								<tr data-index="${idx.index }">
									...
									<td style="">${n.id }</td>
									<td style="">${n.order.orderNo}</td>
									<td style="">${n.dish.name}</td>
									<td style="">${n.amount }</td>
									<td style="">
										<c:if test="${n.type == 1}">菜品</c:if>
										<c:if test="${n.type == 2}">桶装水</c:if>
									</td>
								</tr>
								</c:forEach>
							</tbody>
...
function add() {
		layer.open({
		    ...
		    content: "${ctx}/admin/orderItem/detail",
		    btn: ['确定', '取消'],
...
function edit() {
	...
	layer.open({
	    ...
	    skin: 'layui-layer-rim', //加上边框
	    area: [layer_default_width, layer_default_height], //宽高
	    content: "${ctx}/admin/orderItem/detail?id=" + id,
	    btn: ['确定', '取消'],
	    yes: function(index, layero){ //或者使用btn1
	    	var detailForm = layer.getChildFrame('form', index);
	    	console.log(detailForm);
...
function del() {
	...
	layer.confirm('确认要删除吗?', {icon: 3, title:'提示'}, function(index){
		...
		$.ajax({
			url: "${ctx}/admin/orderItem/delete",
			data: {
				ids: ids.join(',')
			},
			success: function(data) {

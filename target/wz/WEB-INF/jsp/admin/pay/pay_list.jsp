            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-content">
                    	<form role="form" class="form-inline" id="query-form" method="post" action="${ctx }/admin/pay/list" >
                    		<!-- 查询开始 -->
                            ...
                            <div class="form-group">
                                <label for="query-payNO" class="sr-only">系统内编号</label>
                                	<input type="text" placeholder="系统内编号" id="query-payNO" name="payNO" value="${query.payNO }" class="form-control">
                            </div>
                            <div class="form-group">
                                <label for="query-wxPayNo" class="sr-only">微信交易流水号</label>
                                	<input type="text" placeholder="微信交易流水号" id="query-wxPayNo" name="wxPayNo" value="${query.wxPayNo }" class="form-control">
                            </div>
                            <div class="form-group">
						...
						<div>
							<p>
								<sec:authorize ifAnyGranted="pay">
								<!-- 
						...
						<table id="table" data-toggle="table" data-height="400" data-click-to-select="true" data-mobile-responsive="true">
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-field="customer.nickName">用户</th>
                                            <th data-field="payNO">系统内编号</th>
                                            <th data-field="wxPayNo">微信交易流水号</th>
                                            <th data-field="amount">应付金额</th>
                                            <th data-field="actualPayAmount">实付金额</th>
                                            <th data-field="type">交易类型</th>
                                            <th data-field="card">关联会员</th>
                                            <th data-field="createTime">创建时间</th>
                                            <th data-field="payUpdateTime">支付更新时间</th>
							...
							<tbody>
								...
								<tr data-index="${idx.index }">
									...
									<td style="">${n.payNO }</td>
									<td style="">${n.wxPayNo }</td>
									<td style="">${n.amount }</td>
									<td style="">${n.actualPayAmount }</td>
									<td style="">
									<c:if test="${n.type == 1}">购买菜品会员卡</c:if>
									<c:if test="${n.type == 2}">购买桶装水会员卡</c:if>
									</td>
									<td style="">${n.card.name}</td>
									<td style="">
										<fmt:formatDate value="${n.createTime }" pattern="yyyy-MM-dd HH:mm:ss"/>
									</td>
									<td style="">
										<fmt:formatDate value="${n.payUpdateTime }" pattern="yyyy-MM-dd HH:mm:ss"/>
									</td>
...
function edit() {
	...
	layer.open({
	    ...
	    skin: 'layui-layer-rim', //加上边框
	    area: [layer_default_width, layer_default_height], //宽高
	    content: "${ctx}/admin/pay/detail?id=" + id,
	    btn: ['确定', '取消'],
	    yes: function(index, layero){ //或者使用btn1
	    	var detailForm = layer.getChildFrame('form', index);
	    	console.log(detailForm);

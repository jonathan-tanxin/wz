...
<main class="p15 pb100" >
	<div class="fx mb20 bacWhite h100 p10">
		...
		<ul class="fxFl1">
			<li class="f32 pb30">${customer.nickName }</li>
            <li class="f24 fx fxJc-sb"><span >菜品剩余可购：${customer.dishLeft }斤</span><span>桶装水剩余可购：${customer.waterLeft }斤</span></li>
		</ul>
	</div>
	...
	<div class="fx mb20 bacWhite p10 fxFw-w" v-for="c in categories">
	   ...
	   <ul class="f24 w">
	   	...
	   	<section>
	   		<i class="fa fa-plus-square-o mr10" @click="d.num++"></i>
	   		<input class="w40 h40 tc f24 mr10" type="text" v-model="d.num" >
	   		<i class="fa fa-minus-square-o" @click="if (d.num > 0) d.num--"></i>
	   	</section>
	   	</li>
	   </ul>
	</div>
...
<div class="hw-overlayMain">
	   ...
	   <ul>
	    ...
	    <span>{{d.name}}</span><em>{{d.num}}{{d.unit}}</em></li>
	   	...
	   	<li class="fx fxJc-sb"><span>合计</span><em>{{totalDish()}}斤</em></li>
	   	<li class="fx fxJc-r"><em>当前可购斤数：${customer.dishLeft }斤</em></li>
	   </ul>
	   </div>
	   <div>
	   <div class="f20 bbt">桶装水</div>
	   <ul>
	    <template v-for="c in categories">
	    <li class="fx fxJc-sb" v-for="d in c.dishes" v-if="d.num > 0 && c.isWater">
	    <span>{{d.name}}</span><em>{{d.num}}{{d.unit}}</em></li>
	    </template>
	   	<li class="fx fxJc-sb"><span>合计</span><em>{{totalWater()}}桶</em></li>
	   	<li class="fx fxJc-r"><em>当前可购桶数：${customer.dishLeft }桶</em></li>
	   </ul>
	   </div>
	   <div>

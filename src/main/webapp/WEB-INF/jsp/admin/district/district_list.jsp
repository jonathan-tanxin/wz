            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-content">
                    	<form role="form" class="form-inline" id="query-form" method="post" action="${ctx }/admin/district/list" >
                    		<!-- 查询开始 -->
                            <div class="form-group">
                                <label for="query-name" class="sr-only">地区名称</label>
                                	<input type="text" placeholder="地区名称" id="query-name" name="name" value="${query.name }" class="form-control">
                            </div>
                            <div class="form-group">
                                <label for="query-city" class="sr-only">城市</label>
                                	<input type="text" placeholder="城市" id="city-name" name="city.name" value="${query.city.name }" class="form-control">
                            </div>
                            <div class="form-group">
                                <label for="query-letter" class="sr-only">字母</label>
                                	<input type="text" placeholder="字母" id="query-letter" name="letter" value="${query.letter }" class="form-control">
                            </div>
                            <div class="form-group">
...
function add() {
		layer.open({
		    ...
		    content: "${ctx}/admin/district/detail",
		    btn: ['确定', '取消'],
...
function edit() {
	...
	layer.open({
	    ...
	    skin: 'layui-layer-rim', //加上边框
	    area: [layer_default_width, layer_default_height], //宽高
	    content: "${ctx}/admin/district/detail?id=" + id,
	    btn: ['确定', '取消'],
	    yes: function(index, layero){ //或者使用btn1
	    	var detailForm = layer.getChildFrame('form', index);
	    	console.log(detailForm);

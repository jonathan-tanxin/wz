            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-content">
                    	<form role="form" class="form-inline" id="query-form" method="post" action="${ctx }/admin/profitDetail/list" >
                    		<!-- 查询开始 -->
                            <div class="form-group">
                                <label for="query-type" class="sr-only">返利类型</label>
                                	<input type="text" placeholder="返利类型" id="query-type" name="type" value="${query.type }" class="form-control">
                            </div>
                            <div class="form-group">
						...
						<div>
							<p>
								<sec:authorize ifAnyGranted="profitDetail">
						...
						<table id="table" data-toggle="table" data-height="400" data-click-to-select="true" data-mobile-responsive="true">
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-field="type">返利类型</th>
                                            <th data-field="customer">用户</th>
                                            <th data-field="profit">返利id</th>
                                            <th data-field="profitTime">返利时间</th>
                                            <th data-field="status">状态</th>
                                            <th data-field="amount">返利金额</th>
                                            <th data-field="phase">当前返的是第几期</th>
                                            <th data-field="totalPhases">总共多少期</th>
							...
							<tbody>
								...
								<tr data-index="${idx.index }">
									...
									<td style="">
										<fmt:formatDate value="${n.profitTime }" pattern="yyyy-MM-dd HH:mm:ss"/>
									</td>
...
function add() {
		layer.open({
		    ...
		    content: "${ctx}/admin/profitDetail/detail",
		    btn: ['确定', '取消'],
...
function edit() {
	...
	layer.open({
	    ...
	    skin: 'layui-layer-rim', //加上边框
	    area: [layer_default_width, layer_default_height], //宽高
	    content: "${ctx}/admin/profitDetail/detail?id=" + id,
	    btn: ['确定', '取消'],
	    yes: function(index, layero){ //或者使用btn1
	    	var detailForm = layer.getChildFrame('form', index);
	    	console.log(detailForm);
...
function del() {
	...
	layer.confirm('确认要删除吗?', {icon: 3, title:'提示'}, function(index){
		...
		$.ajax({
			url: "${ctx}/admin/profitDetail/delete",
			data: {
				ids: ids.join(',')
			},
			success: function(data) {
